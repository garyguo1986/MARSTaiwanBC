page 50008 "Note Payable Journals"
{
    // +--------------------------------------------------------------+
        ...
        area(navigation)
        {
            ...
            group("&Payments")
            {
                ...
                action(SuggestEmployeePayments)
                {
                    ...
                    PromotedIsBig = true;
                    Visible = false;
                    Enabled = false;  //TWN: new action in BC, maybe need show in TWN
                    //ToolTip = 'Create payment suggestions as lines in the payment journal.';
                ...
                action(PrintCheck)
                {
                    ...
                    trigger OnAction()
                    var
                        NPCheckPrint: Report "Computer Check";
                    begin
                        //Start
                        ...
                        NPCheckPrint.SETTABLEVIEW(GenJnlLine);
                        NPCheckPrint.SetNoteType('NP');
                        NPCheckPrint.RUNMODAL;
                        //Stop
                ...
                group("Electronic Payments")
                {
                    ...
                    Visible = false;
                    Enabled = false; //TWN: new action group in BC, maybe need show in TWN
            ...
            group("P&osting")
            {
                ...
                action(Post)
                {
                    ...
                    begin
                        ...
                        LocalCheckManagementL.PostCheckJnlLine(Rec, FALSE, 0, 'NP');
                        COMMIT;
                        //Stop
                ...
                action("Post and &Print")
                {
                    ...
                    begin
                        ...
                        //Stop
                        LocalCheckManagementL.PostCheckJnlLine(Rec, TRUE, 0, 'NP');
                        COMMIT;
                        CurrentJnlBatchName := GetRangeMax("Journal Batch Name");
                        SetJobQueueVisibility();
                        CurrPage.Update(false);
            ...
            group("Action1000000013")
            {
                action("Print Receipt")
                {
                    ...
                    var
                        ...
                        recCashReport: Report "Check Receipt";
                    begin
                        //Start 
                        CLEAR(recPurchaseSetup);
                        recPurchaseSetup.RESET;

                        recGenJnlLine.COPY(Rec);
                        CLEAR(recGenJnlLine);
                        recGenJnlLine.RESET;
                        recGenJnlLine.SETFILTER(recGenJnlLine."Journal Template Name", Rec."Journal Template Name");
                        recGenJnlLine.SETFILTER(recGenJnlLine."Journal Batch Name", Rec."Journal Batch Name");
                        recGenJnlLine.SETFILTER(recGenJnlLine."Bank Payment Type", FORMAT("Bank Payment Type"::"Computer Check"));
                        ...
                        recGenJnlLine.RESET;
                        recGenJnlLine.COPY(Rec);
                        recGenJnlLine.COPYFILTERS(Rec);
                        recGenJnlLine.SETFILTER(recGenJnlLine."Journal Template Name", Rec."Journal Template Name");
                        recGenJnlLine.SETFILTER(recGenJnlLine."Journal Batch Name", Rec."Journal Batch Name");

                        IF recPurchaseSetup.GET THEN BEGIN
                            IF recPurchaseSetup."Check Receipt Report ID" = 0 THEN BEGIN
                                ERROR(Text002);
                            END ELSE BEGIN
                                IF recGenJnlLine.FIND('-') THEN BEGIN
                                    REPORT.RUNMODAL(recPurchaseSetup."Check Receipt Report ID", TRUE, FALSE, recGenJnlLine);
                                END;
                            END;
                        END;
                        //Stop
    ...
    begin
        ...
        GenJnlManagement.TemplateSelection(PAGE::"Note Payable Journals", 4, false, Rec, JnlSelected);
    ...
    var
        ...
        Text002: Label 'Please setup the Check Receipt Report ID!';
        Text003: Label 'Check needs to be printed first!';
        ...
        DimVisible7: Boolean;
        DimVisible8: Boolean;
        C_BSS_ERR001: Label 'This is a generated line from the cash register.\You must not delete/modify this line.';
