page 1044864 "Posted Inv. With Vehicle Check"
{
    // +----------------------------------------------+
    ...
    // 121695       RGS_TWN-7876   GG   2020-06-10  Add page caption extension

    Caption = 'Posted Inv. With Vehicle Check';
    DeleteAllowed = false;
    InsertAllowed = false;
    PageType = Worksheet;
    ...
    actions
    {
        area(processing)
        {
            action("查詢")
            {
                ...
                begin
                    // Start 119897
                    GetAllPostedInvWithVehicleCheck2;
    ...
    begin
        // Start 119897
        //SetCubeType(2);
        VehicleCheckInvoiceFilters(ServiceCenterFilterG, StartDateG, EndDateG, CalculateDateG);
        // Stop  119897
        // Start 121695
    ...
    var
        ...
        ExptInvedWithCheckG: XMLport "Expt Inved. Veh. With Check";
    ...
    [Scope('OnPrem')]
    procedure GetAllPostedInvWithVehicleCheck2()
    var
    ...
    begin
        // Start 119897
        ExptInvedWithCheckG.SetParam(ServiceCenterFilterG, StartDateG, EndDateG, CalculateDateG, '');
        ExptInvedWithCheckG.MakeDataSource(Rec);
        // Stop  119897
    end;
    ...
    begin
        ...
        IF CURRENTCLIENTTYPE = CLIENTTYPE::Web THEN
            ServiceFilePathL := STRSUBSTNO('PostedInvVehCheck_%1-%2.csv', FORMAT(StartDateG, 0, '<year4><month,2>'), FORMAT(EndDateG, 0, '<year4><month,2>'))
        ELSE BEGIN
            ClientFilePathL := FileMgtL.SaveFileDialog(C_Text100,
                STRSUBSTNO('PostedInvVehCheck_%1-%2', FORMAT(StartDateG, 0, '<year4><month,2>'), FORMAT(EndDateG, 0, '<year4><month,2>')),
                'CSV files (*.csv)|*.csv');
        ...
        ExptInvedWithCheckG.SetParam(ServiceCenterFilterG, StartDateG, EndDateG, CalculateDateG, ClientFilePathL);
        ExptInvedWithCheckG.SETDESTINATION(OutstremL);
        ExptInvedWithCheckG.EXPORT;
        FileL.CLOSE;
        IF CURRENTCLIENTTYPE = CLIENTTYPE::Web THEN
